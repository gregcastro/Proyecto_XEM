...
web_server_address = 'http://10.0.1.206:8000/api'
...
def main():
	...
	while True:
		...
		try:
			...
			gpu_info["rig_hashrate"] = resp[2].split(';')[0]
			...
			for i in range(0, N):
				...
				aux["Fan"] = str(array_fan[i])
				aux["Model"] = "null"
				rig_gpu_info_eth.append(json.dumps(aux))

			gpu_info["rig_gpu_info_eth"] = json.dumps(rig_gpu_info_eth)

			# print(gpu_info["rig_gpu_info_eth"])
			...
			if (len(array_pools) == 1):
				...
				for mhs in array_mhs:
					aux["Mhs"] = mhs
					rig_mhs_second_coin.append(json.dumps(aux))

				gpu_info["rig_gpu_info_second_coin"] = json.dumps(rig_mhs_second_coin)
...
def read_miner_uuid():
	...
	if os.path.exists(config_rig_dir+r"\\rig_uuid.txt"):
		...
		while True:
			...
			r = requests.get(web_server_address+'/rigs/'+rig_uuid, data={"gpu_info":str(gpu_info)})
...
def post_process():
	try:
		...
		# print('\n########################\n')
		r = requests.post(web_server_address+'/rigs/', data=gpu_info)
